...
class Tab extends React.Component {
    ...
    render() {
        ...
        React.createElement("div", { className: b('tab', { active, disabled }), title: stringTitle, onClick: disabled ? undefined : this.onClick }, title));
    }
}
export class AdaptiveTabs extends React.Component {
    get activeTab() {
    ...
    constructor(props) {
        ...
        this.selectSwitcherNode = React.createRef();
        this.tabsRootNode = React.createRef();
        this.tabsListNode = React.createRef();
        ...
        this.renderSwitcher = (switcherProps) => {
            ...
            const title = (React.createElement("div", { className: b('switcher-tab-content') },
                React.createElement("span", { className: b('switcher-tab-text') }, text),
                React.createElement("span", { className: b('switcher-tab-icon') },
                    React.createElement(Icon, { className: b('chevron-icon'), data: ChevronDown }))));
            const switcherTabProps = { title, hint: text, id: 'switcher-tab' };
            const tabItemNode = React.createElement(Tab, Object.assign({}, switcherTabProps, { active: Boolean(active) }));
            return (
        ...
        this.renderTabItem = (item, tabIndex) => {
            ...
            const tabNode = React.createElement(Tab, Object.assign({}, item, { active: item.id === activeTabID }));
            return (
            // eslint-disable-next-line jsx-a11y/click-events-have-key-events, jsx-a11y/no-static-element-interactions
            React.createElement("div", { key: item.id, style: { maxWidth: items.length > 1 ? maxWidth : '100%' }, className: b('tab-container', { 'last-tab': isLastTab, 'no-overflow': noOverflow }), onClick: (e) => {
                    if (!item.disabled) {
    ...
    setUpOverflownTabs(firstHiddenTabIndex, _activeTabIndex, tabsRootNodeWidth) {
        ...
        const overflownAndHiddenTabsKeys = difference(overflownTabsKeys, overflownAndVisibleTabsKeys);
    ...
    renderSelect() {
        ...
        return (React.createElement(Select, { onUpdate: this.onChooseTabFromSelect, options: itemsForSelect, value: [], filterable: false, renderControl: this.renderSwitcherForMoreSelect, onOpenChange: this.handleOpenSelectChange }));
    }
    renderTabsAsSelect() {
        const activeTabID = this.activeTab;
        const { items } = this.props;
        ...
        return (React.createElement(Select, { className: b('tabs-as-select-control'), onUpdate: this.handleTabsAsSelectUpdate, options: itemsForSelect, value: activeTabID === undefined ? [] : [activeTabID], filterable: false, renderControl: this.renderSwitcherForTabsAsSelect, onOpenChange: this.handleOpenSelectChange }));
    }
    render() {
        const { items, className, size = 'm' } = this.props;
        return (React.createElement("div", { ref: this.tabsRootNode, className: b({
                size: size,
                visible: this.state.dimensionsWereCollected,
            }, [className]) }, this.state.currentContainerWidthName === SMALL_CONTAINER_WIDTH_NAME &&
            items.length > 1 ? (this.renderTabsAsSelect()) : (React.createElement(React.Fragment, null,
            React.createElement("div", { ref: this.tabsListNode, className: b('tabs-list') }, this.renderTabs()),
            React.createElement("div", { ref: this.selectSwitcherNode, className: b('select-switcher'), style: { insetInlineStart: this.calcSelectSwitcherInlineStartPosition() } }, this.renderSelect())))));
    }
}
